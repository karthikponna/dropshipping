...
check("page type is a hard constraint, not a hint", () => {
  ...
  assert.match(landing, /components\/Pricing\.tsx/);
  ...
  assert.equal(product.includes("components/Pricing.tsx"), false);
...
check("the user turns carry what each mode needs", () => {
  ...
  const repair = buildRepairMessage({
    pageType: "landing",
    missing: ["components/Pricing.tsx"],
    files: { "components/Hero.tsx": HERO },
    theme: DEFAULT_THEME,
  });
  assert.match(repair, /MISSING FILES/);
  assert.match(repair, /- components\/Pricing\.tsx/);

...
await check("missing files trigger exactly one targeted repair pass", async () => {
  const partial = LANDING_REQUIRED_FILES.filter(
    (path) => path !== "components/Testimonials.tsx" && path !== "components/CTA.tsx",
  );
  const model = fakeModel([
    { payload: payloadFor(partial) },
    {
      payload: payloadFor(["components/Testimonials.tsx", "components/CTA.tsx"], {
        withMeta: false,
        withTheme: false,
      }),
    },
  ...
  assert.match(repairPrompt, /- components\/Testimonials\.tsx/);
...
await check("a chosen model is carried through", async () => {
  const parsed = parseGenerateRequestBody({
    pageType: "landing",
    prompt: "soy candles",
    model: "claude-opus-5",
...
await check("a dated snapshot id is still a valid choice", async () => {
  const parsed = parseGenerateRequestBody({
    pageType: "landing",
    prompt: "soy candles",
...
await check("no model means the app default, not an empty string", async () => {
  for (const model of [undefined, null, ""]) {
    const parsed = parseGenerateRequestBody({ pageType: "landing", prompt: "x", model });
    assert.equal("model" in parsed, false, `${String(model)} should leave the key off entirely`);
  }
});

// The id reaches Anthropic's `model` field, so anything that is not shaped like
// one of their ids is a client bug worth naming rather than a 404 to decode.
await expectBadRequest("a model from another vendor", {
  pageType: "landing",
  prompt: "x",
  model: "gpt-4o",
});
await expectBadRequest("a model id with a path in it", {
  pageType: "landing",
  prompt: "x",
  model: "claude-sonnet-5/../../admin",
});
await expectBadRequest("a non-string model", { pageType: "landing", prompt: "x", model: 5 });
